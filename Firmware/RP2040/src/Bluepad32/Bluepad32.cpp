...
static void controller_data_cb(uni_hid_device_t* device, uni_controller_t* controller) {
    ...
    gp_in.charging = false;  // We cannot reliably detect charging from Bluepad32
    ...
    if (device->controller_type == CONTROLLER_TYPE_PS4Controller && device->report_parser.set_lightbar_color != NULL) {
        ...
        if (combo_held) {
            ...
            // Battery-based indicator: only override for low battery warning
            uint8_t battery_pct = (gp_in.battery * 100) / 255;
    ...
    if (combo_held) {
        // Clear START button and R2 trigger so they don't reach the console
        gp_in.buttons &= ~gamepad->MAP_BUTTON_START;
        gp_in.trigger_r = 0;
        // Also clear D-pad during combo to prevent menu navigation
        gp_in.dpad = 0;
    }

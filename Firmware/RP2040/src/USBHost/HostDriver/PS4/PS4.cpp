...
void PS4Host::process_report(Gamepad& gamepad, uint8_t address, uint8_t instance, const uint8_t* report, uint16_t len)
{
    ...
    if (buttons_released != 0)
    {
        ...
        {
            // Keep previous button state (ignore the release)
            in_report_.buttons[0] = prev_in_report_.buttons[0];
            in_report_.buttons[1] = prev_in_report_.buttons[1];
            in_report_.buttons[2] = prev_in_report_.buttons[2];
        }

...
void PS5Host::process_report(Gamepad& gamepad, uint8_t address, uint8_t instance, const uint8_t* report, uint16_t len)
{
    ...
    if (buttons_released != 0)
    {
        ...
        // Note: For PS5 we can't modify the const in_report, so we skip updating prev_in_report_
    ...
    // DS5 doesn't have analog buttons, but simulate them for compatibility
